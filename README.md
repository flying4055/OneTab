...
- `OneTabIconCache`：站点图标缓存（IndexedDB数据库，替代原来的 favicon_cache_v1）。
- `wallpaper_v1`：壁纸 DataURL。
- `tab_state_v1`：侧边栏 tab 激活状态。

**缓存机制**
- **三级缓存架构**：内存缓存（最快）→ IndexedDB缓存（持久化）→ HTTP缓存（浏览器原生）
- **智能去重**：避免同一时间重复请求相同图标
- **并发控制**：限制同时进行的网络请求数量
- **预加载机制**：在空闲时间预加载可见区域图标
- **自适应策略**：根据不同图标类型采用不同的缓存策略
